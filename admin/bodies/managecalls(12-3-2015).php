...
$qry_calls = "SELECT * FROM calls WHERE gsm_number IN( ".$client_id." ) AND callerid NOT IN( '043423233','0551084153','0558189901','0551084156','0556096251','0508502436','0561756900','0528272154' ) AND test_data=0 AND forward_number!='' ORDER BY id DESC "; 
...
$sql = "SELECT DAYNAME(atr.call_start) as dayname,count(*) as total FROM week_days wd LEFT JOIN (SELECT * FROM calls WHERE call_start >= '".$this_week_sd."' AND call_start <= '".$this_week_ed."' AND gsm_number IN( ".$client_id." ) AND test_data=0   ) atr ON wd.week_day_num = DAYOFWEEK(atr.call_start)	GROUP BY DAYOFWEEK(atr.call_start)";
...
<script type="text/javascript">
	...
	$maxDays=date('t');
		for($i=1;$i<=$maxDays;$i++){ $dayName = date('Y-m-'.$i);
			...
			$qry_calls_month = "SELECT count(*) as month_calls_total FROM calls WHERE call_start LIKE '%".$dayName."%' AND gsm_number IN( ".$client_id." ) AND test_data=0";
...
var last_thirty_data = [
	<?php 
		...
		foreach($arr_calls as $arr_call){
			$i++;
			$qry_calls_month = "SELECT count(*) as month_calls_total FROM calls WHERE call_start LIKE '%".$arr_call."%' AND gsm_number IN( ".$client_id." ) AND test_data=0"; 
			$res_calls_month = $db->Execute($qry_calls_month);
	?>
